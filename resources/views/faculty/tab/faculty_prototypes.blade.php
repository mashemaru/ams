<div class="card-header" id="headingFacultyPrototypes" data-toggle="collapse" data-target="#collapseFacultyPrototypes" aria-expanded="false" aria-controls="collapseFacultyPrototypes">
    <div class="row">
        <div class="col">
            <h5 class="mb-0">PROTOTYPES (SINCE 2005)</h5>
        </div>
...
<div id="collapseFacultyPrototypes" class="collapse" aria-labelledby="headingFacultyPrototypes" data-parent="#accordionExample3">
    <div class="col text-right pt-4 mb-4">
        <!-- Button trigger modal -->
        <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#FacultyPrototypesModal"
            style="float:right">
    ...
    <div class="card-body px-0">
        <div class="table-responsive">
            <table class="table align-items-center table-flush">
                ...
                <tbody>
                @foreach($faculty->faculty_prototypes as $data)
                <tr>
                    <td>{{ $data->author }}</td>
                    <td>{{ $data->title }}</td>
...
<!-- Modal -->
<div class="modal fade" id="FacultyPrototypesModal" tabindex="-1" role="dialog" aria-labelledby="FacultyPrototypesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content bg-secondary">
            <div class="modal-header">
                <h5 class="modal-title" id="FacultyPrototypesModalLabel">Add Prototype</h5>
            ...
            <form method="post" action="{{ route('faculty.store', $faculty->id) }}" autocomplete="off">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group{{ $errors->has('pages') ? ' has-danger' : '' }} mb-3">
                        ...
                        <div class="form-group{{ $errors->has('issn_isbn') ? ' has-danger' : '' }} mb-3">
                        <label class="form-control-label" for="input-issn/isbn">{{ __('ISSN/ISBN') }}</label>
                        <input type="text" name="issn_isbn" id="input-issn/isbn" class="form-control form-control-alternative{{ $errors->has('issn_isbn') ? ' is-invalid' : '' }}" placeholder="{{ __('ISSN/ISBN') }}" value="{{ old('issn_isbn') }}">
                        @if ($errors->has('issn_isbn'))
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $errors->first('issn_isbn') }}</strong>
                            </span>
                        @endif
                    </div>
                </div>
                <div class="modal-footer">
                    ...
                    <button type="submit" name="prototypes" class="btn btn-primary">Add</button>
                </div>
            </form>
        </div>
    </div>
